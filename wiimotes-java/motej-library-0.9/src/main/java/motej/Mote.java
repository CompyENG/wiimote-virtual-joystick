...
public class Mote {
	...
	public Mote(final String bluetoothAddress, Set<String> btAC) {
		try {
		    bluetoothAddressCache = btAC;
			...
			addMoteDisconnectedListener(new MoteDisconnectedListener<Mote>(){
				public void moteDisconnected(MoteDisconnectedEvent<Mote> evt) {
					...
					disconnect();
					if(bluetoothAddressCache.contains(bluetoothAddress)) {
					    System.out.println("bluetoothAddressCache contains... removing!");
					    bluetoothAddressCache.remove(bluetoothAddress);
				    }
				}
			});
	...
	public Set<String> getBluetoothAddressCache() { return bluetoothAddressCache; }
